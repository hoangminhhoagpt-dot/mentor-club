#!/usr/bin/env node
/*
 * trien-khai.mjs — Triển khai 1 lệnh module YouTube cho 1 khách / học viên mới.
 *
 ...
 *            → KIỂM CHỨNG Actions đã bật thật chưa → chạy init-tables + sync-youtube
 *            → in sẵn cấu hình automation cho Lark.
 *
 * Secrets phải mã hoá sealed box (chuẩn GitHub). Có libsodium-wrappers thì script tự set;
 * không có thì in ra để bạn dán tay 30 giây — không chặn quy trình.
 */
import fs from 'fs';
...
const SOURCE = 'hoangminhhoagpt-dot/mentor-club-youtube';
...
for (const k of ['GITHUB_PAT', 'LARK_APP_ID', 'LARK_APP_SECRET', 'LARK_BASE_ID', 'YOUTUBE_API_KEY', 'YT_CHANNEL'])
  if (!C[k] || /^\(|xxx/i.test(String(C[k]))) die(`Thiếu hoặc chưa điền trường "${k}" trong khach.config.json`);

const PAT = C.GITHUB_PAT.trim();
const H = { Authorization: `Bearer ${PAT}`, Accept: 'application/vnd.github+json', 'User-Agent': 'trien-khai-yt', 'Content-Type': 'application/json' };
...
const REPO = (C.REPO_NAME || 'mentor-club-youtube').trim();
const SLUG = `${OWNER}/${REPO}`;
console.log(`\n== TRIỂN KHAI MODULE YOUTUBE → ${SLUG} ==\n`);
...
if (repo.status === 200) {
  ...
  if (repo.status !== 200) die('Fork xong nhưng chưa thấy repo — chờ 1 phút rồi chạy lại.');
  console.log(`  ✔ đã fork → ${SLUG}`);
}

// ---------- 2. Variables (không cần mã hoá) ----------
console.log('\n[2/6] Đặt Variables…');
const VARS = {
  LARK_APP_ID: C.LARK_APP_ID,
  LARK_BASE_ID: C.LARK_BASE_ID,
  YT_CHANNEL: C.YT_CHANNEL,
  ...(C.YT_OAUTH_CLIENT_ID ? { YT_OAUTH_CLIENT_ID: C.YT_OAUTH_CLIENT_ID } : {}),
  ...(C.LARK_DOMAIN ? { LARK_DOMAIN: C.LARK_DOMAIN } : {}),
  ...(C.YT_PRIVACY ? { YT_PRIVACY: C.YT_PRIVACY } : {}),
};
...
// ---------- 3. Secrets (sealed box) ----------
console.log('\n[3/6] Đặt Secrets…');
const SECRETS = {
  LARK_APP_SECRET: C.LARK_APP_SECRET,
  YOUTUBE_API_KEY: C.YOUTUBE_API_KEY,
  ...(C.YT_OAUTH_CLIENT_SECRET ? { YT_OAUTH_CLIENT_SECRET: C.YT_OAUTH_CLIENT_SECRET } : {}),
  ...(C.YT_OAUTH_REFRESH_TOKEN ? { YT_OAUTH_REFRESH_TOKEN: C.YT_OAUTH_REFRESH_TOKEN } : {}),
  ...(C.LARK_NOTIFY_WEBHOOK ? { LARK_NOTIFY_WEBHOOK: C.LARK_NOTIFY_WEBHOOK } : {}),
};
let sodium = null;
...
if (sodium) {
  ...
  console.log('  ! Không có libsodium-wrappers → dán tay các Secret này (30 giây):');
  ...
  console.log('    (Muốn script tự set: chạy "npm install" trong thư mục trien-khai/)');
}

// ---------- 4. Bật Actions (nút bấm tay — GitHub không có API cho việc này) ----------
...
console.log('  ✔ Actions đã bật — init-tables đang chạy, 3 bảng sắp hiện trong Base.');

// ---------- 5. Đổ dữ liệu ----------
console.log('\n[6/6] Chạy sync-youtube (lấy kênh + video về Base)…');
await sleep(25000);                 // chờ init-tables tạo xong bảng rồi mới đổ dữ liệu vào
await dispatch('sync-youtube');
...
Còn 1 việc cuối — dựng automation "Đăng video" trong Lark Base:

  Cò kích hoạt : Khi nhấn nút [Đăng] ở bảng 16.3
                 (hoặc: Khi bản ghi khớp điều kiện → [Đăng] là [Đăng ngay])
  Hành động    : Gửi yêu cầu HTTP
    ...
    Body    {"event_type":"dang-video-youtube","client_payload":{"record_id":"<chèn biến cột Record ID>"}}

  Chi tiết + automation đồng bộ theo lịch: xem LARK-AUTOMATION.md
