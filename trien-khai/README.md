...
#   … điền giá trị của khách …
node trien-khai.mjs
```

Script tự làm: fork repo → đặt Variables → đặt Secrets → **kiểm chứng Actions đã bật thật chưa**
→ chạy `init-tables` (tạo 3 bảng) → chạy `sync-youtube` → in sẵn cấu hình automation cho Lark.
...
Bỏ qua `npm install` cũng chạy được — chỉ là các Secret sẽ được in ra để bạn dán tay.
